...
enum {
    UPSTREAM_PK = MLDSA_PUBLICKEYBYTES(MLD_CONFIG_PARAMETER_SET),
    UPSTREAM_SK = MLDSA_SECRETKEYBYTES(MLD_CONFIG_PARAMETER_SET),
    UPSTREAM_SIG = MLDSA_BYTES(MLD_CONFIG_PARAMETER_SET),
...
#define LEVEL             MLD_CONFIG_PARAMETER_SET
...
_Static_assert(MLDSA_DOMAIN_SEPARATION_MAX_BYTES == UPSTREAM_DS,
               "ML-DSA domain-separation buffer drift");
_Static_assert(MLDSA_PREHASH_NONE == UPSTREAM_PREHASH_NONE, "MLD_PREHASH_NONE drift");

static void *const probes[] = {
    (void *)&GEM_FN(keypair),
    (void *)&GEM_FN(keypair_internal),
    (void *)&GEM_FN(signature),
    (void *)&GEM_FN(signature_internal),
    (void *)&GEM_FN(verify),
    (void *)&GEM_FN(pk_from_sk),
    (void *)&GEM_FN(signature_extmu),
    (void *)&GEM_FN(verify_extmu),
    (void *)&GEM_FN(prepare_domain_separation_prefix),
};
const void *CAT(pqcrypto_conformance_mldsa, LEVEL)(void) {
    return probes;
}
